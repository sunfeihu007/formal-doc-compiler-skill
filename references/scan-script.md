...
Run via Bash. Requires `pyyaml`. Install once: `pip install pyyaml --break-system-packages`.

## Usage

```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/compliance-check/references/scan.py \
    --doc path/to/draft.docx \
    --wordlist .compliance/wordlist.yaml
```

Output is JSON on stdout — one hit per line plus a final summary line.

## The script

Save as `scan.py` alongside this file (the plugin install will include it).
...
def scan(text: str, wordlist: dict) -> list:
    ...
    for cat, spec in wordlist.get("categories", {}).items():
        ...
        for term in spec.get("terms", []):
            # treat any term containing regex metacharacters as a regex
...
Each non-summary line is a JSON object with `category`, `term`, `matched`, `context`, `suggested_replacement`. Parse and present as a table.

The summary line has `summary: true` plus counts. Use it to decide whether to proceed (hits == 0) or block delivery (hits > 0).
